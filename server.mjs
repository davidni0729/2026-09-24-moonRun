...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost');
 if(u.pathname==='/events'){res.writeHead(200,{'Content-Type':'text/event-stream','Cache-Control':'no-cache','Connection':'keep-alive'});const token=u.searchParams.get('token')||'';streams.set(res,token);try{game.player(token).connected=true;}catch{}res.write('data: '+JSON.stringify(game.state(token))+'\n\n');req.on('close',()=>{streams.delete(res);if(![...streams.values()].includes(token))try{game.player(token).connected=false;}catch{}});return;}
 ...
 const buf=await fs.readFile(full);res.writeHead(200,{'Content-Type':({'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.png':'image/png'})[path.extname(full)]||'application/octet-stream','Cache-Control':'no-store'});res.end(buf);
